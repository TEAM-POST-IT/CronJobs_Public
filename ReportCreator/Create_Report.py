...
# vectorizer load
tv = pickle.load(open("tfidf_vectorizer.p", "rb"))
        ...
        most_error.append(
            {
                ...
                "count": view_count,
                "title": row["title"],
                "creation_date": "{:%Y-%m-%d %H:%M:%S}".format(
                    datetime.fromtimestamp(row["creation_date"])
                ),
            }
        )
...
for i, category in enumerate(category_count):
    all_category_ratio.append({"name": category_name_dict[i], "value": category})
...
for category in range(7):
    ...
    for tag in tag_list:
        ...
        for i in range(3):
            child.append({"name": child_list[i], "value": child_count[i]})

        root_children.append({"name": tag, "children": child})
    ...
    for i in range(10):
        most_hot_keyword.append({"word": word_list[i], "count": count_list[i]})
    ...
    for i in range(len(word_list)):
        tag_wordcloud.append({"word": word_list[i], "count": count_list[i]})
