...
for i in range(1, 4):
    ...
    res = urlopen(
        "https://tech.kakao.com/blog/page/" + str(i) + "/#posts", context=context
    )
    ...
    for url in urls:
        if len(date[n - 1].text) < 3:  # 몇개의 게시물의 날짜가 표시되지 않는다, 게시글까지 타고 들어가서 날짜를 가져온다
            ...
            arr.append(
                {
                    "title": url.text,
                    "url": atag[n - 1].get("href"),
                    "date": date_branch[0].text,
                    "image": img[n - 1]["src"],
                }
            )
            # print(url.text + " " +
...
for url in urls:
    ...
    resdate = (
        str(subdate[subdate.find(",") + 2 : subdate.find(",") + 6])
        + "."
        + str(dayFarmat(dateMatcher[subdate[0:3]]))
        + "."
        + str(dayFarmat(subdate[4 : subdate.find(",")]))
    )

    arr.append(
        {
            "title": url.text,
            "url": "http://woowabros.github.io" + atag[n - 1].get("href"),
            "date": resdate,
            "image": "https://www.woowahan.com/img/pc/common-logo.png",
            "category": 2,
        }
    )
    ...
    arr.append(
        {
            ...
            "url": atag[n - 1].get("href"),
            "date": "20" + resDate,
            "image": "https://line.me/static/940874c48d2369be137d812b15491843/f2838/icon-title-pc.png",
            "category": 4,
        }
    )
    # print(url.text +" "+ atag[n-1].get('href') + " " + resDate)
...
for url in urls:
    ...
    dateText = date[n - 1].text
    date_split = dateText.split(" ")

    month = dayFarmat(time.strptime(date_split[0][:-1], '%b').tm_mon)
    day = dayFarmat(date_split[1][:-1].strip())
    year = dayFarmat(date_split[2].strip())
...
imgs = soup.select("div.u-lineHeightBase > a")
...
for url in urls:
    ...
    if len(dateText) < 8:
        ...
        day = dayFarmat(
            dateText[dateText.find(",") - 2 : dateText.find(",")].replace(" ", "0")
        )
    month = dateMatcher[dayFarmat(dateText[0:3])]
    resDate = year + "." + month + "." + day
    ...
    arr.append(
        {
            ...
            "url": atag[n - 1].get("href"),
            "date": resDate,
            "image": imgs[n - 1].get("style").split('"')[1],
            "category": 6,
        }
    )
    # print(url.text + " " + atag[n-1].get('href') + " " + resDate)
...
last_week = datetime.today() - timedelta(1) # 일일 단위 크롤링
...
print("[+] Success insert blog in a week")
